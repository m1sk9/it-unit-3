...
第 8 章から C++ が登場する．C++ のソースは `.cpp` 拡張子で置く．

- 規格は **C++17** を基準にする（実験室の `g++` バージョンは未確認のため暫定．判明したら `.clangd` の該当行を直す）．
- `.clangd` は `If.PathMatch` で C（`.c` / `.h`）と C++（`.cpp` / `.hpp` / `.cc`）を分岐済み．C ファイルは `-xc -std=c11`，C++ ファイルは `-xc++ -std=c++17` で解釈される．`.cpp` を C と誤認させない．
...
C++ のソースは `g++-15` と `-std=c++17` でコンパイルする：

```fish
g++-15 -std=c++17 -o main src/8/report_1.cpp
./main
```
...
`src/<章>/<課題名>.c`（C）または `src/<章>/<課題名>.cpp`（C++）の単位でファイルを置く．1 ファイル＝独立した main を持つ単体プログラム．
