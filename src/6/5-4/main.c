...
static void summarize(const char *sumname) {
    ...
    for (int id = 0; id < MAX_JOB; id++) {
        ...
        int min_prog = -1, max_prog = 0; /* ★5-4 追加: ジョブ内の最短/最長プログラム時間の初期化 */
        ...
        while (fgets(line, sizeof(line), job)) {
            ...
            if (code == 'N') { /* Nの処理 */
                ...
                int prog_time = u_sec - l_sec; /* ★5-4 変更: プログラム時間を変数に保持 */
                prog_count++;                  /* プログラム数++ */
                total_prog += prog_time;       /* 総プログラム時間に加算 */
                if (min_prog < 0 || prog_time < min_prog)
                    min_prog = prog_time; /* ★5-4 追加: 最短プログラム時間の更新 */
                if (prog_time > max_prog)
                    max_prog = prog_time; /* ★5-4 追加: 最長プログラム時間の更新 */
            }
        }
        int job_time = f_sec - n_sec; /* ジョブ時間算出 */

        /* ジョブ時間・プログラム数・総プログラム時間・最短/最長プログラム時間をサマリへ出力 */
        fprintf(sum, "%d %d %d %d %d\n", /* ★5-4 変更: 末尾2列（最短・最長プログラム時間）を追加 */
                job_time, prog_count, total_prog, prog_count ? min_prog : 0, max_prog);
        fclose(job);
    }
    fclose(sum);
}
...
static void report(const char *sumname, const char *repname) {
    ...
    int min_job = -1, max_job = 0;     /* ★5-4 追加: 最短/最長ジョブ時間の初期化 */
    int min_ptime = -1, max_ptime = 0; /* ★5-4 追加: 最短/最長プログラム時間の初期化 */
    int jt, pc, tp;
    int mn, mx; /* ★5-4 追加: サマリ末尾2列の読み込み用 */

    /* ジョブ情報の処理*（サマリ1行＝1ジョブ） */
    while (fscanf(sum, "%d %d %d %d %d", &jt, &pc, &tp, &mn, &mx) ==
           5) {          /* ★5-4 変更: 5列読み込みに変更 */
        n_jobs++;        /* ジョブ数++ */
        total_job += jt; /* ジョブ時間を抽出して加算 */
        n_prog += pc;
        total_prog += tp;
        if (min_job < 0 || jt < min_job)
            min_job = jt; /* ★5-4 追加: 最短ジョブ時間の更新 */
        if (jt > max_job)
            max_job = jt; /* ★5-4 追加: 最長ジョブ時間の更新 */
        if (pc > 0) {     /* ★5-4 追加: プログラムを持つジョブのみ集計 */
            if (min_ptime < 0 || mn < min_ptime)
                min_ptime = mn; /* ★5-4 追加: 最短プログラム時間の更新 */
            if (mx > max_ptime)
                max_ptime = mx; /* ★5-4 追加: 最長プログラム時間の更新 */
        }
    }
    ...
    fprintf(
        rep,
        "Shortest Job Time = %d(sec.) Longest Job Time = %d(sec.)\n", /* ★5-4 追加:
                                                                         最短/最長ジョブ時間の出力
                                                                       */
        min_job < 0 ? 0 : min_job, max_job);
    /* プログラム情報出力（平均時間 = 総プログラム時間 / 総プログラム数） */
    fprintf(rep, "Number Of Programs = %d Average Time = %d(sec.)\n", n_prog,
            n_prog ? total_prog / n_prog : 0);
    fprintf(
        rep,
        "Shortest Program Time = %d(sec.) Longest Program Time = %d(sec.)\n", /* ★5-4 追加:
                                                                                 最短/最長プログラム時間の出力
                                                                               */
        min_ptime < 0 ? 0 : min_ptime, max_ptime);
